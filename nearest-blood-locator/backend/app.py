...
#     'blood_bank': f"mysql+pymysql://{DB_USER}:{DB_PASS_QUOTED}@{DB_HOST}:{DB_PORT}/blood_locator"
# }
...
#         conn.close()
#         print(f"âœ… Database '{DB_NAME}' and 'blood_bank' are ready")
#     except Exception as e:
#         print(f"âŒ Database creation error: {e}")
#         raise
...
#         print(f"📝 Registration attempt - Username: {username}, Email: {email}, Role: {role}, BloodGroup: {blood_group}")
...
#         if role in ['donor', 'recipient'] and not blood_group:
#             return jsonify({"msg": f"Blood group is required for {role}s"}), 400
...
#             if not blood_group:
#                 db.session.rollback()
#                 return jsonify({"msg": "Blood group required for donors"}), 400
...
#             )
#             db.session.add(donor)
#             print(f"✅ Donor profile created for user {user.id}")

#         elif role == 'recipient':
#             if not blood_group:
#                 db.session.rollback()
#                 return jsonify({"msg": "Blood group required for recipients"}), 400
...
#             )
#             db.session.add(recipient)
#             print(f"✅ Recipient profile created for user {user.id}")
...
#             )
#             db.session.add(bank)
#             print(f"✅ Blood bank profile created for user {user.id}")

#         # Commit everything
#         db.session.commit()
#         print(f"🎉 Registration successful for {username} (ID: {user.id})")
...
def register():
    try:
        ...
        if role == 'donor':
            ...
            print(f"🔍 Creating recipient profile:")
            print(f"   user_id: {user.id}")
            print(f"   name: {username}")
            print(f"   required_blood_group: {blood_group}")
            print(f"   phone: {phone}")
            print(f"   city: {city}")
        ...
        # Commit everything
        db.session.commit()
        print(f"🎉 Registration successful for {username} (ID: {user.id}, Role: {role})")
...
if __name__ == "__main__":
    ensure_database_exists()
    with app.app_context():
        # db.drop_all()  # ⚠️ This will delete existing data
        # db.create_all()
